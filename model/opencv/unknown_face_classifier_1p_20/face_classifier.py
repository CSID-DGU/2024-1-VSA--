...
class FaceClassifier():
    ...
    def draw_name(self, frame, face):
        ...
        if face.name != "person_01":
            ...
            # Apply mosaic (resize down and back up)
            mosaic_scale = 0.1  # Adjust this value for mosaic granularity
            ...
            # Replace the face region with the mosaic
            frame[top:bottom, left:right] = mosaic
        else:
            # Draw box and name for person_01
            width = 20
            if width > (right - left) // 3:
                width = (right - left) // 3
            height = 20
