...
def mtcnn_detection(model, img, device):
    ...
    #print(f"Detected bboxes: {bboxes}, type: {type(bboxes)}") # 디버깅 메시지지
    return bboxes
...
def mtcnn_recognition(face_db, unknown_embeddings, recog_thr, similarity_metric='euclidean'):
    """
    Recognize faces using embeddings and the chosen similarity metric.
    """
    face_ids = []
    probs = []
    ...
    for i in range(len(unknown_embeddings)):
        ...
        for name, known_embeddings in face_db.items():
            for known_eb in known_embeddings:
                if similarity_metric == 'euclidean':
                    score = (eb - known_eb).norm().item()  # Euclidean distance
                elif similarity_metric == 'cosine':
                    score = F.cosine_similarity(eb.unsqueeze(0), known_eb.unsqueeze(0)).item()  # Cosine similarity

                if similarity_metric == 'euclidean' and score < best_score:
                    best_score = score
                    best_match = name
                elif similarity_metric == 'cosine' and score > best_score:
                    best_score = score
                    best_match = name

        # 코사인 유사도에서는 쓰레시홀드보다 큰 값을 동일 인물로 판단
        if similarity_metric == 'cosine' and best_score > recog_thr:
            face_ids.append(best_match)
        elif similarity_metric == 'euclidean' and best_score < recog_thr:  # 유클리디안 거리
            face_ids.append(best_match)
        else:
...
#     #print(f"Recognition result: {face_ids}")  # 디버깅 메시지 추가    
